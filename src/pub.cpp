 ...
 public:
  Pub() : Node("Publisher"), count_(0) {
    ...
    service = this->create_service<beginner_tutorials::srv::InputStr>(
          "service_node", std::bind(&Pub::input_string, this, std::placeholders::_1,
                    std::placeholders::_2));
    }
    catch(...){
    ...
    timer_ =
        this->create_wall_timer
                    (std::chrono::milliseconds(int((1000 / freq))), std::bind(&Pub::timer_callback, this));
  }
  void input_string(const std::shared_ptr<beginner_tutorials::srv::InputStr::Request> request,
          std::shared_ptr<beginner_tutorials::srv::InputStr::Response> response){
    response->out_msg = request->inp_msg;
    new_str = response->out_msg;
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Incoming request: %s", request->inp_msg.c_str());
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Sending back response: %s", response->out_msg.c_str());
  }
 private:
  void timer_callback() {
